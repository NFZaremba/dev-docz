...
> _Implicit Coercion_ is when `JavaScript` handles the type conversion for you behind the scenes (_using the same functions above_). _Implicit coercion_ will mostly take place in an expression with operands of different types.
