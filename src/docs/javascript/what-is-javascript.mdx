...
Originally, it was only **interpreted**, but modern JS engines like V8, Spidermonkey, and Nitro use various techniques to perform **Just-in-Time Compilation (JIT)** for better performance.
...
JS is dynamic weakly typed language, meaning you do not explicitly annotate variableswith types (`string`, `int`, etc) and the true types are **not known until runtime**.
...
**Single-Threaded** means that JS can only run one instruction at a time, even if your CPU has multiple cores and available threads. So how does JS handle jobs concurrently?
