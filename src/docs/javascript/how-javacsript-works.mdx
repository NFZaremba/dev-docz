...
In web development, each browser has a **JS Runtime Environment**, and in that runtime is a **JavaScript Engine** that parses the code.
...
A JavaScript engine is a program or an interpreter which **parses and executes JavaScript code**.
...
When running a JS program, two regions of memory are allocated on the machine's **RAM** (Random Access Memory), to store things like the `runtime`, `variables` and `objects` that you reference in your code. The `Heap` and `Stack`.

---

### Runtime Environment

Provides additional scripts/features/API's at runtime to build JavaScript based software.
